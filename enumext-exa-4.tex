...
\begin{enumext}[save-ans=simplews,columns=2,check-ans=true,show-ans=true,nosep,save-ref=true,wrap-label={\tikz[scale=0.25]\duck[signpost=\scalebox{0.6}{#1}];}]
  \item Factor $x^{2}-2x+1$ %%%\anskey{GG}

        \begin{scontents}[print-env=true, store-env=simplews]
      ZZZZZZZ
    \end{scontents}
  ...
  \item Factor $3x+3y+3z$ \anskey{$3(x+y+z)$} %%\anskey{GG}
  \item True False
    \begin{enumext}[nosep]
      \item $\alpha > \delta$ \anskey{False} %%\anskey{False}
