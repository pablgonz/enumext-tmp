...
Before enumext AAA %%%%%%
\begin{enumext}[columns=1,nosep,itemsep=0pt]%,]%[topsep=0pt,parsep=0pt,partopsep=0pt,itemsep=0pt,columns=1]%,,itemsep=0pt
...
\begin{enumext*}[columns=5,columns-sep=3pt,nosep]%,nosep,topsep=20pt,parsep=10pt
     \item P \item Q \item X  \item R \item S
  \end{enumext*}
...
\item Using [columns=2,mini-env] here HHHH
  \begin{enumext}[columns=1,mini-env=0.4\linewidth,itemsep=0.71mm]%,topsep=10pt,parsep=30pt
     \item  P \item Q \item Q  \item R \item S
    \miniright
    \rule{30pt}{20pt}\par
    P Tikz image here
