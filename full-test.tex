...
\begin{enumext}[nosep,itemsep=0pt,align=center,save-ans=mytest,check-ans=true,columns=2,save-ref,widest=99,] %show-ans
...
\begin{enumext}[save-ans=new,show-ans,check-ans, save-ref,nosep,,ref={[\S\Roman*]},before=\setenumext[keyans]{font=\small,list-offset=0cm,wrap-label={\textcolor{gray}{##1}},nosep}]
...
\begin{enumext}[wrap-label={\tikz[scale=0.4]\duck[signpost=\scalebox{0.6}{#1}];},save-ans=Worksheet,check-ans,columns=2,nosep,save-ref]%
...
\begin{enumext}[wrap-label={\tikz[scale=0.4]\duck[signpost=\scalebox{0.6}{#1}];},save-ans=Worksheet,resume,check-ans,columns=2,nosep,save-ref]%
\item Factor $x^{2}-2x+1$  \anskey{$\left(x-1\right)^{2}$}
\item Factor $3x+3y+3z$    \anskey{$3(x+y+z)$}
\item True False
  \begin{enumext}
...
\begin{enumext}[save-ans=mytest,resume, check-ans,save-ref]
